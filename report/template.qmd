...
if (spec$plots$revision_history$enabled) {
	...
	dt_table <- function(data, group_cols = 2, i_row_name = 1, digits = spec$plots$revision_history$table$digits,
						 caption = NULL) {
	...
	rev_data <- lapply(basename(list_files), function(f){
		res <- lapply(studied_methods, function(method) {
			read.ts(file.path(dirname(params$file), spec$dataset, method, f), list = TRUE)
		})
...
for (serie in names(all_data[[1]])) {
	...
	if (spec$plots$bymethods$enabled){
		...
		cat("\n\n::::: {.panel-tabset}\n")
		for (methods in names(last_est_s)) {
			cat(sprintf("\n### %s\n\n", gsub("\n", " ", methods)))
	...
	if (spec$plots$comparison_plot$enabled) {
		...
		data_comp <- do.call(ts.union, lapply(last_est_s, `[[`, "tc"))
		first_date <- time(last_est_s[[1]]$tc)[length(last_est_s[[1]]$tc)] - spec$plots$nyears
		range_y <- range(unlist(lapply(last_est_s, function(x) range(window(ts.union(x$tc), start = first_date)))))
		cat("\n\n::::: {.panel-tabset}\n")
	...
	if (spec$plots$revision_history$enabled) {
		...
		cat("\n\n::::: {.panel-tabset}\n")
		for (methods in names(rev_data_s)) {
			cat(sprintf("\n### %s\n\n", gsub("\n", " ", methods)))
